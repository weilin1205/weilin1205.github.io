...
function createtime1() {
  var grt = new Date("07/28/2022 00:00:00"); //建站時間
  now1.setTime(now1.getTime() + 250);
  var days = (now1 - grt) / 1000 / 60 / 60 / 24;
  var dnum = Math.floor(days);

  var ascll = [
    `Welcome to Weilin's Blog`,
    `🎯🎯🎯 Keep going 🔥🔥🔥`,
    `
...
function createtime2() {
  ...
  setTimeout(console.log.bind(console, "%c WELCOME %c 歡迎光臨", "color:white; background-color:#23c682", ""));

  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by William Wen %c 您正在訪問 Weilin's Blog",
  ...
  setTimeout(
    console.warn.bind(console, "%c S013-782 %c 您現在正處於監控中", "color:white; background-color:#d9534f", "")
  );
}
createtime2();
...
if (localStorage.getItem("font") == undefined) {
  localStorage.setItem("font", "default");  // 預設字體 (目前設 default)，如果想改預設字體，可以改成 "LXGW" 等 (在下方 <h2>三、字體設置 - Font Settings</h2> 的都行)
}
