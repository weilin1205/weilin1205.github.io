    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "⚠️您正在使用F12⚠️",
            message: "若要使用網站原始碼，請務必遵守GPL協議以及清楚來源註明！",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "warning",
    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "切換右鍵模式成功🍔",
            message: "目前滑鼠右鍵已更換為網站指定樣式！\n[備註：若您有安裝強制右鍵插件，建議將其關閉以避免樣式衝突]",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "success",
...
function createtime2() {
  var ascll2 = [`NCC2-036`, `使用前置鏡頭拍照成功，識別為「大聰明」`, `Photo captured: `, ` 🤪 `];
  ...
  setTimeout(console.log.bind(console, "%c WELCOME %c 歡迎光臨，大聰明", "color:white; background-color:#23c682", ""));
  ...
  setTimeout(console.log.bind(console, "%c W23-12 %c 系統監測到你已打開控制台", "color:white; background-color:#4f90d9", ""));
  setTimeout(
    console.warn.bind(console, "%c S013-782 %c 你現在正處於監控中", "color:white; background-color:#d9534f", "")
  );
}
createtime2();
    ...
    setTimeout(() => {
      new Vue({
        data: function () {
          this.$notify({
            title: "已切換至夜間模式！🌙",
            message: "Switched to Night Mode!🌆",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "success",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Weilin's Blog") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText(url);
    // navigator.clipboard.writeText('Fomalhaut🥝的站內分享\n標題：' + subTitle + '\n連結：' + url + '\n歡迎來訪！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功複製分享連結🎉",
          message: "連結已成功複製到剪貼簿，您現在可以直接與朋友們分享了！",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
...
if (m == 12 && dd == 5) {//站長生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝站長" + (y - 2003).toString() + "歲生日快樂！🎉");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("07/28/2022 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://img.shields.io/badge/現在時光正好-努力奮鬥吧~-6adea8?style=social&logo=cakephp' title='珍惜時間，抓緊快樂啦~'><br> <div style="font-size:13px;font-weight:bold">本站居然運行了 ${dnum} 天 ${hnum} 小時 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i></div>`
      : `<img class='boardsign' src='https://img.shields.io/badge/夜深了-休息是為了走更長的路-6adea8?style=social&logo=coffeescript' title='這個時間點該去睡覺啦，熬夜對身體不好哦，晚安，好夢~'><br> <div style="font-size:13px;font-weight:bold">本站居然運行了 ${dnum} 天 ${hnum} 小時 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i></div>`),
...
if (window.localStorage.getItem("fpson") == undefined || window.localStorage.getItem("fpson") == "1") {
  ...
  var loop = function () {
    ...
    if (now > 1000 + lastTime) {
      ...
      if (fps <= 5) {
        var kd = `<span style="color:#bd0000">非常卡盪</span>`
      } else if (fps <= 15) {
        var kd = `<span style="color:red">輕度卡盪</span>`
      } else if (fps <= 25) {
        var kd = `<span style="color:orange">幀率低下</span>`
      } else if (fps < 35) {
        var kd = `<span style="color:#9338e6">不太流暢</span>`
      } else if (fps <= 45) {
        var kd = `<span style="color:#08b7e4">幀率尚可</span>`
      } else {
        var kd = `<span style="color:#39c5bb">十分流暢</span>`
      }
  ...
  setTimeout(function () {
    new Vue({
      data: function () {
        this.$notify({
          title: "提示🍒",
          message: " (｡･∀･)ﾉﾞ由於網站部分設置項更新，目前已為您重置所有設置，祝您愉快！",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
...
function setBlurNum() {
  ...
  target.innerHTML = "模糊半徑 (開啟模糊生效 0px-100px): " + newBlur + "px";
...
// 随机美图_古風壁紙cdn.seovx.com
let seovxTradition = "url(https://cdn.seovx.com/ha/?mom=302)";
// picsum随机
...
// suyanw
// let suyanw = "url(https://api.suyanw.cn/api/comic/api.php)";
// btstu正妹壁紙，暫時使用suyanw的變數
let suyanw = "url(https://api.btstu.cn/sjbz/api.php)";
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://raw.githubusercontent.com/weilin1205/weilin1205.github.io/master/img/ACG/ACG006.jpg);
    --darkmode-bg:url(https://raw.githubusercontent.com/weilin1205/weilin1205.github.io/master/assets/ACG006.jpg);
...
function createWinbox() {
  ...
  winbox = WinBox({
    id: "meihuaBox",
    index: 99,
    title: "美化設置主控台 - Beautify Setting Console",
    x: "left",
    y: "center",
    minwidth: "300px",
    height: "60%",
...
<br>
<center><p><button onclick="reset()" style="background:linear-gradient(to right, #fc354c, #0abfbc);display:block;width:40%;padding:15px 0;border-radius:30px;color:white;font-size:1.1em;"><i class="fa-solid fa-arrows-rotate"></i>&nbsp;恢復全部默認設置</button></p></center>

<h2>一、顯示偏好 - Display Settings</h2>
...
<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 星空特效 (夜間模式) </div><input type="checkbox" id="universeSet" onclick="setUniverse()">
  <div class="content-text" style="font-weight:bold; padding-left:20px"> 霓虹彩燈 (夜間模式) </div><input type="checkbox" id="lightSet" onclick="setLight()">
</div>

<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 模糊效果 (本地耗能) </div><input type="checkbox" id="blur" onclick="setBlur()">
  <div class="content-text" style="font-weight:bold; padding-left:20px"> 側邊工具 (默認開啟) </div><input type="checkbox" id="rightSideSet" onclick="toggleRightside()">
</div>

<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 幀率監測 (刷新生效) </div><input type="checkbox" id="fpson" onclick="fpssw()">
...
<div class="content" style="display:flex;padding-left:12px"><input type="radio" id="red" name="colors" value=" "
        ...
        onclick="setColor('puregreen')"><input type="radio" id="blue" name="colors" value=" " 
        ...
        onclick="setColor('darkblue')"><input type="radio" id="purple" name="colors" valFue=" "
        onclick="setColor('purple')"><input type="radio" id="pink" name="colors" value=" "
        onclick="setColor('purepurple')"><input type="radio" id="pink" name="colors" value=" " 
        onclick="setColor('pink')" checked="checked"><input type="radio" id="black" name="colors" value=" "
        onclick="setColor('gray')"><input type="radio" id="black" name="colors" value=" " 
...
<center><button onclick="resetBg()" style="background:var(--theme-color);display:block;width:35%;padding:15px 0;border-radius:30px;color:white;"><i class="fa-solid fa-arrows-rotate"></i>&nbsp;回復默認背景</button></center>
...
<details class="folding-tag" cyan><summary> 查看風景背景 </summary>
              <div class='content'>
...
<details class="folding-tag" cyan><summary> 查看寵物背景 </summary>
              <div class='content'>
...
<h3>5. 漸層色</h3>
<details class="folding-tag" cyan><summary> 查看漸層色背景 </summary>
              <div class='content'>
...
<h3>6. 純色</h3>
<details class="folding-tag" cyan><summary> 查看純色背景，支援自定義顏色 </summary>
              <div class='content'>
...
<h3>7. 適配手機</h3>
<details class="folding-tag" cyan><summary> 查看適配手機的背景 </summary>
              <div class='content'>
...
<h3>8. 自定義背景</h3>
<details class="folding-tag" cyan><summary> 設置您的自定義背景 </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="請輸入有效的圖片連結，如 https://source.fomal.cc/img/home_bg.webp"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🔃🌄切換背景🌄🔃</button></center></p>
              </div>
            </details>
